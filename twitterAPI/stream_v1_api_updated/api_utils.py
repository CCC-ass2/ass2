...
def processed_json_dict(city_name,district_name,json_object):
    my_dict = {}
    my_dict['id'] = json_object['id']
    my_dict['location'] = city_name
    my_dict['district'] = district_name
...
def get_city_district_from_json(json_object,city_boundary_dict):
    # print(json_object)
    ...
    if 'place' in json_object and json_object['place']!=None:
        print(json_object['place'])
        if json_object['place']['place_type'] == 'city':
            print('place used')
            district = json_object['place']['name']
            city_name,district_name = city_boundary_dict.location_check(district)
    if city_name==None:
        city_name,district_name = city_boundary_dict.location_check(location)

    if city_name == None:
        city_name,district_name = city_boundary_dict.coordinate_check(geo)

    if city_name == None:
        city_name,district_name = city_boundary_dict.coordinate_check(coordinate)



    return city_name,district_name

def stream_write_to_file(json_object,f,simp_f,blog,other_f,with_location_f,n,city_boundary_dict):
    if 'limit' in json_object:
        print('reach tracker limit')
        return

    city_name,district_name = get_city_district_from_json(json_object,city_boundary_dict)

    my_dict = processed_json_dict(city_name,district_name, json_object)
    ...
    simp_f.flush()
    if city_name!=None:
        json.dump(my_dict,with_location_f)
        with_location_f.write('\n')
        with_location_f.flush()

        blog.write("time:{0}\n".format(datetime.datetime.now()))
        blog.write("No.{},id:{}\n".format(n,json_object["id_str"]))
        blog.flush()
        print(my_dict['id'],' collected')
    elif my_dict['geo']!=None or my_dict['coordinates']!=None:
        print('with coordinates, save to others')
        json.dump(my_dict,other_f)
        other_f.write('\n')
        other_f.flush()
...
class my_stream(tweepy.Stream):
    ...
    def preprocess(self,result_file_path,geojson_path,city_name_file_path):
        ...
        self.validated_f = open(result_file_path+'/with_location.txt', 'a+', encoding='utf8')
        self.other_f = open(result_file_path+'/other.txt', 'a+', encoding='utf8')
        self.city_dict = city_boundaries(geojson_path,city_name_file_path)
        self.n = 0
    ...
    def on_data(self, raw_data):
        ...
        self.n+=1
        print("No.",self.n)
        stream_write_to_file(json_object, self.f, self.simp_f, self.blog,self.other_f,self.validated_f,self.n,self.city_dict)
...
def time_process(time_str,city_name):
    ...
    time_zone = time_str.split(' ')[4]
    time_zone_num = int(time_zone) / 100
